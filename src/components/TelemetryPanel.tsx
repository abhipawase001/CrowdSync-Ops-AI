    ...
    <section aria-labelledby="telemetry-heading" className="rounded-2xl border border-slate-800 bg-slate-950/60 p-5">
      ...
      <ul className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {gates.map((g) => {
          ...
          const warn = !critical && g.current_capacity_pct >= 60;
          ...
          return (
            ...
            >
              <div className="flex items-baseline justify-between">
                <span className="font-semibold text-slate-100">{g.gate_id}</span>
                <span
                  className={`text-2xl font-bold tabular-nums ${critical ? "text-red-400" : warn ? "text-amber-300" : "text-emerald-300"}`}
                  aria-label={`${g.current_capacity_pct} percent capacity`}
                >
                  {g.current_capacity_pct}%
                </span>
              </div>
              <div
                className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-slate-800"
                role="progressbar"
                aria-valuenow={g.current_capacity_pct}
                aria-valuemin={0}
                aria-valuemax={100}
              >
                <div
                  className={`h-full ${barColor} transition-all duration-500`}
                  style={{ width: `${g.current_capacity_pct}%` }}
                />
              </div>
              <p className="mt-3 text-xs text-slate-400">
                Inflow <span className="text-slate-200 tabular-nums">{g.inflow_rate_per_min}</span>/min
              </p>
              {g.incident_reported ? (
